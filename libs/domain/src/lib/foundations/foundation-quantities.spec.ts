...
describe('Foundation Quantities Domain Contracts', () => {
  it('should define all 5 material families with valid labels', () => {
    ...
    FOUNDATION_MATERIAL_FAMILIES.forEach((family) => {
      ...
      expect(FOUNDATION_MATERIAL_FAMILY_LABELS[family].length).toBeGreaterThan(0);
    });
  });

  it('should have complete metadata for every field in FOUNDATION_VOLUME_QUANTITY_FIELDS', () => {
    FOUNDATION_VOLUME_QUANTITY_FIELDS.forEach((field: FoundationVolumeQuantityField) => {
  ...
  it('should configure correct default waste factors per RN-12', () => {
    ...
    expect(FOUNDATION_QUANTITY_METADATA_MAP.excavationHardFootingM3.defaultWastePercent).toBe(10);
    expect(FOUNDATION_QUANTITY_METADATA_MAP.excavationNormalFootingM3.defaultWastePercent).toBe(5);
    expect(FOUNDATION_QUANTITY_METADATA_MAP.excavationWaterFootingM3.defaultWastePercent).toBe(20);
    expect(FOUNDATION_QUANTITY_METADATA_MAP.excavationPierM3.defaultWastePercent).toBe(5);

    // RN-12: Concreto 5%
    expect(FOUNDATION_QUANTITY_METADATA_MAP.concreteFootingsM3.defaultWastePercent).toBe(5);
    expect(FOUNDATION_QUANTITY_METADATA_MAP.concretePiersM3.defaultWastePercent).toBe(5);

    // RN-12: Aço de reforço 10%, Aço de tubulão 3%
    expect(FOUNDATION_QUANTITY_METADATA_MAP.steelFootingsKg.defaultWastePercent).toBe(10);
    expect(FOUNDATION_QUANTITY_METADATA_MAP.steelPiersKg.defaultWastePercent).toBe(3);
  });
});
