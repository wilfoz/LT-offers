...
@Component({
  selector: 'app-foundation-volume-history',
  imports: [
    RouterLink,
    DatePipe,
  ...
  template: `
    <section>
      <h2>
        ...
        @if (history(); as h) {
          {{ h.combination.seriesName }} / {{ h.combination.towerCode }} · Solo
          {{ h.combination.soilCode }} · Fundação
          {{ h.combination.foundationCode }}
        }
      </h2>
      <a matButton routerLink="/catalogs/foundation-volumes">
        Voltar à listagem
      </a>
...
export class FoundationVolumeHistoryComponent {
  ...
  readonly history = signal<FoundationVolumeHistory | null>(null);
  ...
  formatQuantities(version: FoundationVolumeVersion): string {
    return quantitiesSummary(version);
  }
}
