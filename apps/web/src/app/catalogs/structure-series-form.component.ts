      ...
      <form [formGroup]="form" (ngSubmit)="save()">
        @if (!editId()) {
          <div>
            ...
            <input id="name" formControlName="name" maxlength="100" />
            @if (errorFor('name')) {
              <p class="error" role="alert">{{ errorFor('name') }}</p>
            }
          </div>
        } @else {
          <p>
        ...
        <div>
          ...
          <input id="designer" formControlName="designer" maxlength="100" />
        </div>

        <div>
          <label for="voltageKv">Tensão (kV)</label>
          <input
            id="voltageKv"
          ...
          @if (errorFor('voltageKv')) {
            <p class="error" role="alert">{{ errorFor('voltageKv') }}</p>
          }
        </div>

        <div>
          <label for="circuitCount">Quantidade de circuitos</label>
          <input
            id="circuitCount"
          ...
          @if (errorFor('circuitCount')) {
            <p class="error" role="alert">{{ errorFor('circuitCount') }}</p>
          }
        </div>

        <div>
          <label for="cablesPerPhase">Cabos por fase</label>
          <input
            id="cablesPerPhase"
          ...
          @if (errorFor('cablesPerPhase')) {
            <p class="error" role="alert">{{ errorFor('cablesPerPhase') }}</p>
          }
        </div>

        <div>
          <label for="designWindSpeedMs">Vento de projeto (m/s)</label>
          <input
          ...
          @if (errorFor('designWindSpeedMs')) {
            <p class="error" role="alert">
              {{ errorFor('designWindSpeedMs') }}
            </p>
          }
        </div>

        <div>
          <label for="insulatorType">Tipo de isolador</label>
          <input
            id="insulatorType"
        ...
        <div>
          <label for="silMw">SIL (MW)</label>
          <input id="silMw" formControlName="silMw" inputmode="decimal" />
          @if (errorFor('silMw')) {
            <p class="error" role="alert">{{ errorFor('silMw') }}</p>
          }
        </div>
        ...
        <a routerLink="/catalogs/structure-series">Cancelar</a>
      </form>
    </section>
  `,
  styles: `
