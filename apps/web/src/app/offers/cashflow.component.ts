...
@Component({
    ...
    <div class="cashflow-container">
      ...
      <div class="header-section">
        <div>
          ...
          <h2 class="title">Desembolso, Cronograma de Entregas & Fluxo de Caixa</h2>
          <p class="description">
            Projeção temporal mês a mês de saídas e faturamento, cronograma de suprimentos e identificação da exposição financeira máxima.
          </p>
        </div>
      ...
      @if (loading()) {
        <div class="loading-panel">
          ...
          <span>Consolidando curvas temporais de desembolso e faturamento...</span>
        </div>
      }
      ...
      @if (!loading() && cashflow(); as cf) {
        ...
        <div class="kpis-grid">
          <div class="kpi-card">
            ...
            <div class="kpi-value text-danger">{{ formatCurrency(cf.totalOutflow) }}</div>
            <div class="kpi-subtext">Materiais, serviços e canteiros</div>
          </div>

          <div class="kpi-card">
            <div class="kpi-label">Faturamento Total (Entradas)</div>
            <div class="kpi-value highlight">{{ formatCurrency(cf.totalInflow) }}</div>
            <div class="kpi-subtext">Medições e adiantamentos</div>
          </div>

          <div class="kpi-card">
            <div class="kpi-label">Saldo Líquido Acumulado</div>
            <div class="kpi-value highlight-green">{{ formatCurrency(cf.finalAccumulatedBalance) }}</div>
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-subtext">
              Ocorre no Mês M{{ cf.financialExposure.peakMonth }} · Giro: {{ formatCurrency(cf.financialExposure.recommendedWorkingCapital) }}
            </div>
          </div>
        </div>
        ...
        <div class="chart-card">
          <div class="chart-header">
            <h3 class="chart-title">Curva Mensal de Desembolso vs Faturamento (RF-57, RF-59)</h3>
            <div class="chart-legend">
              <span class="legend-item"><span class="bullet bar-outflow"></span> Desembolso Mensal (Saídas)</span>
              <span class="legend-item"><span class="bullet bar-inflow"></span> Faturamento Mensal (Entradas)</span>
              <span class="legend-item"><span class="bullet bar-balance"></span> Saldo de Caixa Acumulado</span>
            </div>
          </div>
              ...
              <div class="month-col">
                <div class="bars-pair">
                  <div
                    ...
                    [title]="'Mês ' + pt.month + ' · Desembolso: ' + formatCurrency(pt.totalOutflow)"
                  ></div>
                  <div
                    class="bar-item bar-inflow"
                    [style.height.px]="getBarHeight(pt.totalInflow)"
                    [title]="'Mês ' + pt.month + ' · Faturamento: ' + formatCurrency(pt.totalInflow)"
                  ></div>
                </div>
                <div class="accum-badge" [class.accum-neg]="Number(pt.accumulatedCashflow) < 0">
        ...
        @if (cf.supplyDeliverySchedule && cf.supplyDeliverySchedule.length > 0) {
          <div class="supplies-card">
            <h3 class="table-title">Cronograma de Entregas de Materiais & Ponderação de Commodities (RF-58)</h3>
            <div class="table-responsive">
              <table class="data-table">
                ...
                <tbody>
                  @for (sup of cf.supplyDeliverySchedule; track sup.materialGroup + sup.month) {
                    <tr>
                      <td class="font-bold">{{ sup.materialGroup }}</td>
                      <td class="text-center font-mono">M{{ sup.month }}</td>
                      <td class="text-center font-mono">{{ sup.tonsOrUnits }}</td>
                      <td class="text-center font-mono font-bold">{{ sup.percentage }}</td>
                      <td class="text-right font-mono">{{ formatCurrency(sup.estimatedCost) }}</td>
                    </tr>
                  }
                </tbody>
              </table>
        ...
        <div class="table-card">
          <div class="table-header">
            <h3 class="table-title">Detalhamento Analítico de Fluxo de Caixa (Quadro D / DT)</h3>
            <span class="badge badge-info">{{ cf.totalMonths }} Meses Projetados</span>
          </div>
          ...
          <div class="table-responsive">
            <table class="data-table">
              ...
              <tbody>
                @for (pt of cf.monthlyPoints; track pt.month) {
                  <tr [class.highlight-peak]="pt.month === cf.financialExposure.peakMonth">
                    <td class="text-center font-mono font-bold">
                      M{{ pt.month }}
                      @if (pt.month === cf.financialExposure.peakMonth) {
                        <span class="peak-tag" title="Mês de máxima exposição de caixa">PICO</span>
                      }
                    </td>
                    <td class="text-right font-mono">{{ formatCurrency(pt.materialsOutflow) }}</td>
                    <td class="text-right font-mono">{{ formatCurrency(pt.servicesOutflow) }}</td>
                    <td class="text-right font-mono">{{ formatCurrency(pt.indirectsOutflow) }}</td>
                    <td class="text-right font-mono font-bold text-danger">{{ formatCurrency(pt.totalOutflow) }}</td>
                    <td class="text-right font-mono text-muted">{{ formatCurrency(pt.accumulatedOutflow) }}</td>
                    <td class="text-right font-mono">{{ formatCurrency(pt.advanceBilling) }}</td>
                    <td class="text-right font-mono">{{ formatCurrency(pt.measurementBilling) }}</td>
                    <td class="text-right font-mono font-bold highlight-text">{{ formatCurrency(pt.totalInflow) }}</td>
                    <td class="text-right font-mono text-muted">{{ formatCurrency(pt.accumulatedInflow) }}</td>
                    <td class="text-right font-mono font-bold" [ngClass]="{'text-danger': Number(pt.netMonthlyCashflow) < 0, 'highlight-green': Number(pt.netMonthlyCashflow) > 0}">
                      {{ formatCurrency(pt.netMonthlyCashflow) }}
                    </td>
                    <td class="text-right font-mono font-bold" [ngClass]="{'text-danger': Number(pt.accumulatedCashflow) < 0, 'highlight-green': Number(pt.accumulatedCashflow) >= 0}">
  ...
  styles: [`
    ...
    .bar-outflow { background: #ef4444; }
    .bar-inflow { background: #0284c7; }
    .bar-balance { background: #16a34a; }
...
export class CashflowComponent implements OnInit {
  ...
  loadCashflow(): void {
    ...
    req$.subscribe({
      ...
      error: () => {
        this.errorMessage.set('Erro ao carregar as curvas de desembolso e fluxo de caixa.');
        this.loading.set(false);
      },
    });
  }
