...
@Component({
  ...
  template: `
    <div class="checks-container">
      ...
      <div class="header-section">
        <div>
          <div class="subtitle-badge">Fase F6.1 · Módulo M12 (RF-62, RF-63, RNF-09)</div>
          <h2 class="title">Painel de Verificações de Consistência & Integridade</h2>
          <p class="description">
            Diagnóstico global cruzado em tempo real sobre quantitativos, cotações, cronograma, histogramas, serviços e desembolso.
          </p>
        </div>
      ...
      @if (healthSummary(); as summary) {
          ...
          <div class="status-content">
            <h3 class="status-title">
              @if (summary.status === 'HEALTHY') {
                ...
                Avisos Operacionais Identificados (Fechamento Permitido com Justificativa)
              } @else {
            ...
            <p class="status-desc">
              @if (summary.status === 'HEALTHY') {
                Todas as baterias de validação cruzada foram aprovadas sem qualquer discrepância. A revisão está pronta para fechamento.
              } @else if (summary.status === 'WARNINGS_ONLY') {
                Existem {{ summary.warningCount }} alerta(s) de produção ou déficit operacional. O orçamentista pode fechar a revisão fornecendo justificativa formal.
              } @else {
                Existem {{ summary.criticalCount }} erro(s) crítico(s) que corrompem a integridade dos cálculos. Corrija os itens listados para habilitar o fechamento da revisão.
              }
            </p>
          </div>
        </div>
        ...
        <div class="kpi-grid">
          <div class="kpi-card critical">
            ...
            <span class="kpi-value critical-text">{{ summary.criticalCount }}</span>
          ...
          <div class="kpi-card warning">
            ...
            <span class="kpi-value warning-text">{{ summary.warningCount }}</span>
            <span class="kpi-subtext">Requerem nota formal de turno/locação</span>
          </div>
        ...
        <div class="findings-card">
          <div class="card-header">
            <div class="card-title-row">
              <h3>Lista de Diagnósticos e Apontamentos ({{ filteredFindings().length }})</h3>
          ...
          <div class="findings-list">
            @for (f of filteredFindings(); track f.id) {
              <div class="finding-item" [class.critical-border]="f.severity === 'CRITICAL'" [class.warning-border]="f.severity === 'WARNING'">
                <div class="finding-header">
                  <div class="badges-group">
                    ...
                    >
                      {{ f.severity === 'CRITICAL' ? 'CRÍTICO' : f.severity === 'WARNING' ? 'ALERTA' : 'INFO' }}
                    </span>
                    <span class="rule-badge">{{ f.ruleId }}</span>
                    <span class="module-badge">{{ getModuleLabel(f.module) }}</span>
                    @if (f.lineName) {
              ...
              <div class="empty-state">
                ✓ Nenhuma pendência encontrada nesta categoria! A proposta está em perfeita conformidade.
              </div>
            }
          </div>
        </div>
  ...
  styles: [`
    ...
    .kpi-card.critical { border-color: #fca5a5; background: #fff5f5; }
    .kpi-card.warning { border-color: #fde68a; background: #fffdf5; }
    .kpi-card.info { border-color: #bfdbfe; background: #f8faff; }
    .kpi-label {
      font-size: 0.75rem;
      text-transform: uppercase;
      font-weight: 700;
    ...
    .pill-btn.critical.active { background: #dc2626; border-color: #dc2626; color: #ffffff; }
    .pill-btn.warning.active { background: #d97706; border-color: #d97706; color: #ffffff; }
    .findings-list {
      display: flex;
      flex-direction: column;
    ...
    .finding-item.critical-border { border-left-color: #dc2626; }
    .finding-item.warning-border { border-left-color: #d97706; }
    .finding-header {
      display: flex;
      justify-content: space-between;
      align-items: center;
    ...
    .btn-nav:hover { background: #2563eb; color: #ffffff; }
    .finding-title {
      margin: 0;
      font-size: 0.95rem;
      font-weight: 700;
