...
@Component({
    ...
    <div class="camps-container">
      <div class="header-section">
        <div>
          ...
          <p class="description">
            Dimensionamento orçamentário do Canteiro Central e Canteiros Avançados, custos de terraplenagem/implantação, infraestrutura modular, quadro de pessoal indireto de apoio e desmobilização.
          </p>
        </div>
      ...
      @if (!loading() && campsSummary()) {
        ...
        <div class="kpis-grid">
          <div class="kpi-card">
            ...
            <div class="kpi-value highlight">{{ formatCurrency(campsSummary()?.totalCampsCost) }}</div>
            <div class="kpi-subtext">Implantação + Operação + Desmob.</div>
          </div>

          <div class="kpi-card">
            <div class="kpi-label">Implantação & Módulos</div>
            <div class="kpi-value">{{ formatCurrency(campsSummary()?.totalImplementationCost) }}</div>
            <div class="kpi-subtext">Infraestrutura inicial</div>
          </div>

          <div class="kpi-card">
            <div class="kpi-label">Operação & Manutenção</div>
            <div class="kpi-value">{{ formatCurrency(campsSummary()?.totalOperatingCost) }}</div>
            <div class="kpi-subtext">Aluguéis + Pessoal de Apoio</div>
          </div>

          <div class="kpi-card">
            <div class="kpi-label">Desmobilização</div>
            <div class="kpi-value">{{ formatCurrency(campsSummary()?.totalDemobilizationCost) }}</div>
            ...
            <div class="camp-card">
              <div class="camp-header">
                <div class="camp-title-group">
                  <span class="camp-type-badge" [class.type-central]="camp.type === 'CENTRAL'">
                    {{ camp.type === 'CENTRAL' ? 'Canteiro Central' : 'Canteiro Avançado' }}
                  </span>
                  <h3 class="camp-name">{{ camp.name }}</h3>
                  <span class="camp-location">Estaca / Local: Km {{ camp.locationKm || '0' }} · Período: M{{ camp.startMonth }}..M{{ camp.endMonth }} ({{ camp.durationMonths }} meses)</span>
                </div>
                <div class="camp-total-cost">
                  <div class="cost-label">Custo Total do Canteiro</div>
                  <div class="cost-value">{{ formatCurrency(camp.totalCampCost) }}</div>
                </div>
              </div>
              ...
              <div class="camp-details-grid">
                <div class="detail-box">
                  ...
                  <div class="box-value">{{ formatCurrency(camp.implementationCost) }}</div>
                </div>
                <div class="detail-box">
                  <div class="box-label">Custo Fixo Mensal</div>
                  <div class="box-value">{{ formatCurrency(camp.fixedMonthlyCost) }}/mês</div>
                </div>
                <div class="detail-box">
                  <div class="box-label">Pessoal Mensal</div>
                  <div class="box-value">{{ formatCurrency(camp.totalPersonnelMonthlyCost) }}/mês</div>
                </div>
                <div class="detail-box">
                  <div class="box-label">Desmobilização</div>
                  <div class="box-value">{{ formatCurrency(camp.demobilizationCost) }}</div>
                </div>
              </div>
              ...
              @if (camp.personnel && camp.personnel.length > 0) {
                <div class="personnel-section">
                  <h4 class="personnel-title">Quadro de Pessoal Indireto do Canteiro ({{ camp.personnel.length }} cargos)</h4>
                    ...
                    <tbody>
                      @for (p of camp.personnel; track p.laborRoleId) {
                        <tr>
                          ...
                          <td class="text-right">{{ formatCurrency(p.monthlyUnitCost) }}</td>
                          <td class="text-right font-bold">{{ formatCurrency(p.totalMonthlyCost) }}</td>
                        </tr>
                      }
                    </tbody>
                  </table>
  ...
  styles: [`
    ...
    .kpi-value.highlight { color: #b45309; }
