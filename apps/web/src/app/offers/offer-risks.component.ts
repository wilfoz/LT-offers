...
@Component({
      ...
      <div class="header-section">
        <div>
          <div class="subtitle-badge">
            Fase F6.1 · Módulo M12 (RF-61, RF-55)
          </div>
          <h2 class="title">Matriz de Riscos & Contingências da Proposta</h2>
          <p class="description">
            Identificação e quantificação de incertezas de projeto, cálculo de
            severidade ponderada e alimentação direta das contingências do BDI.
          </p>
        </div>
      ...
      @if (assessment(); as summary) {
        <div class="kpi-grid">
          <div class="kpi-card">
            ...
            <span class="kpi-value"
              >R$ {{ formatCurrency(summary.totalEstimatedImpact) }}</span
            >
          ...
          <div class="kpi-card highlight">
            ...
            <span class="kpi-value highlight-text"
              >R$ {{ formatCurrency(summary.totalWeightedSeverity) }}</span
            >
          ...
          <div class="kpi-card success">
            ...
            <span class="kpi-value success-text"
              >R$ {{ formatCurrency(summary.bdiContingencyAmount) }}</span
            >
          ...
          <div class="kpi-card neutral">
            ...
            <span class="kpi-value"
              >R$ {{ formatCurrency(summary.commercialAssumptionAmount) }}</span
            >
            <span class="kpi-subtext"
              >Absorvido na margem sem onerar edital</span
            >
          </div>
        </div>
            ...
            @for (cat of summary.categoryBreakdown; track cat.category) {
              @if (cat.count > 0) {
                <div class="cat-pill" [attr.data-cat]="cat.category">
                  <strong>{{ getCategoryLabel(cat.category) }}</strong
                  >: {{ cat.count }} item(ns) · R$
                  {{ formatCurrency(cat.totalWeightedSeverity) }}
                </div>
              }
            }
          </div>
        ...
        <div class="table-card">
          <div class="card-header">
            <h3>
              Registro Analítico de Incertezas e Riscos ({{
                ...
                @for (item of summary.items; track item.id) {
                  <tr>
                    ...
                    <td class="text-right num highlight-cell">
                      <strong
                        >R$ {{ formatCurrency(item.weightedSeverity) }}</strong
                      >
                    </td>
                    <td>
                      <span
                        ...
                        [class.commercial]="
                          item.treatment === 'COMMERCIAL_ASSUMPTION'
                        "
                      >
                        {{
                          item.treatment === 'CONTINGENCY_BDI'
                            ? 'Contingência BDI'
                            : 'Premissa Comercial'
                        }}
                      </span>
                  ...
                  <tr>
                    <td colspan="8" class="empty-state">
                      Nenhum risco registrado nesta seleção. Clique em "+ Novo
                      Risco" para adicionar.
                    </td>
                  </tr>
                }
              </tbody>
        ...
        >
          <div class="modal-card">
            <div class="modal-header">
              <h3>
                {{ editingId() ? 'Editar Risco' : 'Cadastrar Novo Risco' }}
              </h3>
            ...
            <div class="modal-body">
              <div class="form-row">
                <div class="form-group col-6">
                  <label for="risk-category">Categoria de Risco *</label>
                  <select
                    id="risk-category"
                  ...
                  >
                    ...
                    <option value="SCHEDULE_TIMELINE">
                      Prazo / Cronograma
                    </option>
                    <option value="WEATHER_RAIN">Clima / Chuva</option>
                    <option value="GEOTECHNICAL_SOIL">Geotécnico / Solo</option>
                    <option value="ENGINEERING_INTERFACE">
                      Engenharia / Interface
                    </option>
                    <option value="THIRD_PARTY_MARKET">
                      Terceiros / Mercado
                    </option>
                ...
                <div class="form-group col-6">
                  <label for="risk-line">Linha de Transmissão</label>
                  <select
                    id="risk-line"
                    [(ngModel)]="formLineId"
                    class="form-control"
              ...
              <div class="form-group">
                <label for="risk-description">Descrição do Risco *</label>
                <input
                  type="text"
                  id="risk-description"
              ...
              <div class="form-group">
                <label for="risk-situation"
                  >Situação / Contexto na Proposta</label
                >
                <textarea
                  id="risk-situation"
              ...
              <div class="form-group">
                <label for="risk-mitigation">Ação de Mitigação Adotada</label>
                <textarea
                  id="risk-mitigation"
              ...
              <div class="form-row">
                <div class="form-group col-4">
                  <label for="risk-impact">Impacto Estimado (R$) *</label>
                  <input
                    type="number"
                    step="1000"
                    id="risk-impact"
                ...
                <div class="form-group col-4">
                  <label for="risk-probability">Probabilidade (%) *</label>
                  <input
                    type="number"
                    min="0"
                    max="100"
                ...
                <div class="form-group col-4">
                  <span class="field-label">Severidade Calculada</span>
              ...
              <div class="form-group">
                <span class="field-label" id="risk-pricing-treatment-label"
                  >Tratamento na Formação de Preço *</span
                >
                <div
                  class="radio-group"
                  role="radiogroup"
                  aria-labelledby="risk-pricing-treatment-label"
                >
                  <label class="radio-label">
                    ...
                    />
                    <strong>Contingência BDI:</strong> Adiciona o valor
                    ponderado à linha de contingência da proposta.
                  </label>
                  <label class="radio-label">
                    <input
                      type="radio"
                      ...
                      [(ngModel)]="formTreatment"
                    />
                    <strong>Premissa Comercial:</strong> Incerteza monitorada
                    assumida na margem sem onerar o edital.
                  </label>
                </div>
              </div>
            </div>
  ...
  styles: [
    `
      ...
      .badge-cat[data-cat='LAND_EASEMENT'] {
        background: #fef3c7;
        color: #92400e;
      }
      .badge-cat[data-cat='ENVIRONMENTAL'] {
        background: #dcfce7;
        color: #166534;
      }
      .badge-cat[data-cat='GEOTECHNICAL_SOIL'] {
        background: #ffedd5;
        color: #9a3412;
      }
      .badge-cat[data-cat='SCHEDULE_TIMELINE'] {
        background: #fee2e2;
        color: #991b1b;
      }
      .badge-cat[data-cat='WEATHER_RAIN'] {
        background: #e0e7ff;
        color: #3730a3;
      }
      .badge-cat[data-cat='THIRD_PARTY_MARKET'] {
        background: #f3e8ff;
        color: #6b21a8;
      }
      .treatment-badge {
...
export class OfferRisksComponent implements OnInit {
  ...
  loadRisks(): void {
    const lineIdStr =
      this.viewScope() === 'LINE' && this.selectedLineId()
  ...
  openAddModal(): void {
    ...
    this.formLineId = this.selectedLineId()
      ? String(this.selectedLineId())
      : undefined;
  ...
  // Fecha apenas no clique direto no backdrop (o card nao propaga mais o clique).
  ...
  deleteRisk(riskId: string): void {
    ...
    this.risksApi
      .deleteRisk(this.offerId, riskId, lineIdStr)
      .subscribe((res) => {
        this.assessment.set(res);
      });
  }
