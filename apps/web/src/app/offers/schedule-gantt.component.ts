    ...
    <div class="schedule-container">
      ...
      <div class="header-section">
        <div>
          ...
          <p class="description">
            Dimensionamento de durações, alocação de equipes de trabalho,
            aplicação do redutor de produtividade por pluviosidade regional
            (RN-16), controle de marcos LI/LO e validação de sobreprodução
            (RN-15).
          </p>
        </div>
      ...
      @if (!loading() && schedule()) {
        ...
        <div class="kpis-grid">
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-value">
              {{ formatCurrency(schedule()?.totalScheduleCost) }}
            </div>
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-value">
              {{ formatCurrency(schedule()?.totalDirectLaborCost) }}
            </div>
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-value">
              {{ formatCurrency(schedule()?.totalIndirectCost) }}
            </div>
        ...
        @if (schedule()?.warnings && schedule()!.warnings.length > 0) {
          <div class="warnings-banner">
            <div class="warning-title">
              <span>⚠️</span> Alertas de Consistência Temporal e Limites de
              Produção ({{ schedule()!.warnings.length }})
            </div>
        ...
        <div class="milestones-section">
          <h3 class="section-title">
            Marcos Contratuais de Transmissão (RF-39)
          </h3>
              ...
              <div class="milestone-chip" [class.mandatory]="m.isMandatory">
                ...
                <div class="milestone-info">
                  ...
                  <span class="milestone-target"
                    >Mês Alvo: <strong>M{{ m.targetMonth }}</strong>
                    {{ m.targetDate ? '(' + m.targetDate + ')' : '' }}</span
                  >
                </div>
              </div>
            }
          ...
          <div class="gantt-wrapper">
            <div class="gantt-table">
              ...
              <div class="gantt-header-row">
                ...
                <div class="gantt-timeline-header">
                  @for (m of monthIndices(); track m) {
                    <div
                      class="month-cell"
                      [class.milestone-col]="isMilestoneMonth(m)"
                    >
                      M{{ m }}
                      @if (getMilestoneForMonth(m); as ms) {
                        <span class="ms-marker" [title]="ms.name">{{
                          ms.code
                        }}</span>
                      }
                    </div>
              ...
              @for (act of filteredActivities(); track act.id) {
                <div
                  class="gantt-row"
                  [class.row-warning]="act.status !== 'PLANNED'"
                >
                  <div class="gantt-col-meta">
                    ...
                    <div class="activity-names">
                      ...
                      <span class="act-group"
                        ...
                        {{ act.assignedCrewName || 'Sem equipe' }}</span
                      >
                    </div>
                  </div>
                  <div class="gantt-col-qty">
                    {{ act.totalQuantity }} {{ act.quantityUnit }}
                  </div>
                  <div class="gantt-col-crews">{{ act.crewCount }} eq.</div>
                  <div class="gantt-col-prod">
                    {{ act.monthlyProduction }}/mês
                  </div>
        ...
        <div class="table-section">
          <h3 class="section-title">
            Memória de Atividades e Custos Temporais
          </h3>
              ...
              <tbody>
                @for (act of filteredActivities(); track act.id) {
                  <tr>
                    ...
                    <td>
                      <span
                        ...
                        [style.background-color]="
                          getGroupColor(act.group) + '22'
                        "
                        [style.color]="getGroupColor(act.group)"
                        >{{ getGroupLabel(act.group) }}</span
                      >
                    </td>
                    <td class="text-right">
                      {{ act.totalQuantity }} {{ act.quantityUnit }}
                    ...
                    <td class="text-right">
                      {{ formatCurrency(act.monthlyRecurringCost) }}
                    </td>
                    <td class="text-right">
                      {{ formatCurrency(act.demobilizationCost) }}
                    </td>
...
export class ScheduleGanttComponent implements OnInit {
    ...
    this.scheduleApi.getLineSchedule(this.lineId).subscribe({
      ...
      error: (err) => {
        this.errorMessage.set(
          err.error?.message ||
            'Falha ao carregar o cronograma físico da linha.',
        );
        this.loading.set(false);
      },
    });
  ...
  isMilestoneMonth(m: number): boolean {
    return (this.schedule()?.milestones || []).some(
      (ms) => ms.targetMonth === m,
    );
  }

  getMilestoneForMonth(m: number): MilestoneContract | undefined {
    return (this.schedule()?.milestones || []).find(
      (ms) => ms.targetMonth === m,
    );
  }
