    ...
    <div class="histogram-container">
      ...
      <div class="header-section">
        <div>
          ...
          <p class="description">
            Projeção mensal de efetivo de mão de obra por função (diretos de
            campo vs indiretos de canteiro), identificação de picos de
            mobilização (RF-43) e balanço de equipamentos próprios vs déficit a
            locar (RN-17, RF-44).
          </p>
        </div>
          ...
          <div class="kpis-grid">
            <div class="kpi-card">
              ...
              <div class="kpi-value highlight">
                {{ data.peakManpower.total }}
                <span class="unit">colaboradores</span>
              </div>
              <div class="kpi-subtext">
                Ocorre no Mês M{{ data.peakManpower.month }}
              </div>
            </div>
            ...
            <div class="kpi-card">
              ...
              <div class="kpi-value">
                {{ data.totalManMonths }} <span class="unit">homem-mês</span>
              </div>
              <div class="kpi-subtext">
                Direto: {{ data.totalDirectManMonths }} · Indireto:
                {{ data.totalIndirectManMonths }}
              </div>
            </div>
        ...
        @if (activeTab() === 'EQUIPMENT') {
          <div class="kpis-grid">
            <div class="kpi-card">
              ...
              <div class="kpi-value highlight">
                {{ data.peakEquipment.total }}
                <span class="unit">equipamentos</span>
              </div>
              <div class="kpi-subtext">
                Próprios: {{ data.peakEquipment.own }} · Locados:
                {{ data.peakEquipment.rented }}
              </div>
            </div>
            ...
            <div class="kpi-card">
              ...
              <div class="kpi-value">
                {{ formatCurrency(data.totalEquipmentRentalCost) }}
              </div>
              <div class="kpi-subtext">
                Déficit comercial atendido por terceiros
              </div>
            </div>
            ...
            <div class="kpi-card">
              ...
              <div class="kpi-value highlight-green">
                {{ totalOwnEquipments() }}
                <span class="unit">unidades próprias</span>
              </div>
        ...
        <div class="chart-section">
          <div class="section-header">
            <h3 class="section-title">
              {{
                activeTab() === 'MANPOWER'
                  ? 'Curva Mensal de Mão de Obra (Efetivo Direto vs Indireto)'
                  : 'Curva Mensal de Equipamentos (Frota Própria vs Locação)'
              }}
            </h3>
          </div>
          ...
          <div class="legend-row">
            @if (activeTab() === 'MANPOWER') {
              <div class="legend-item">
                <span class="bullet bar-direct"></span> Mão de Obra Direta
                (Campo)
              </div>
              <div class="legend-item">
                <span class="bullet bar-indirect"></span> Mão de Obra Indireta
                (Canteiro/Gestão)
              </div>
            } @else {
              <div class="legend-item">
                <span class="bullet bar-own"></span> Frota Própria Disponível
              </div>
              <div class="legend-item">
                <span class="bullet bar-rented"></span> Déficit de Locação
                Comercial
              </div>
            }
          </div>
                ...
                @if (activeTab() === 'MANPOWER') {
                  @for (item of data.manpowerItems; track item.laborRoleId) {
                    <tr>
                      ...
                      <td>
                        <span
                          class="badge"
                          [class.badge-primary]="item.isDirect"
                          [class.badge-neutral]="!item.isDirect"
                          >{{ item.isDirect ? 'Direto' : 'Indireto' }}</span
                        >
                      </td>
                      ...
                      @for (pt of item.monthlyHeadcount; track pt.month) {
                        <td
                          class="text-center"
                          [class.cell-zero]="pt.count === 0"
                        >
                          {{ pt.count || '-' }}
                        </td>
                      }
                      <td class="text-right font-bold">
                        {{ item.totalManMonths }} MM
                      </td>
                    </tr>
                  }
                } @else {
                  @for (eq of data.equipmentItems; track eq.equipmentId) {
                    <tr>
                      ...
                      <td>
                        Próprios: <strong>{{ eq.ownUnitsAvailable }}</strong>
                      </td>
                      ...
                      @for (pt of eq.monthlyDemand; track pt.month) {
                        <td
                          class="text-center"
                          [class.cell-deficit]="pt.deficitToRent > 0"
                        >
                          {{ pt.totalRequired || '-' }}
                          @if (pt.deficitToRent > 0) {
                            <span
                              class="sub-deficit"
                              title="Déficit para locação"
                              >(+{{ pt.deficitToRent }})</span
                            >
                          }
                        </td>
                      }
                      <td class="text-right font-bold">
                        {{ formatCurrency(eq.totalRentalCost) }}
                      </td>
                    </tr>
                  }
                }
