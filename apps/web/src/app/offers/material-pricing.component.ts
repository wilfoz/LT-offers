...
import { PricingApiService, PricingSimulationPayload } from './pricing-api.service';
    ...
    <div class="pricing-container">
      ...
      <div class="header-section">
        <div>
          ...
          <p class="description">
            Formação dinâmica de preços com curva LME/Midwest, conversão cambial, apuração fiscal (ICMS, DIFAL base dupla, FECOEP, IPI, PIS/COFINS) e benefícios REIDI.
          </p>
        </div>
      ...
      @if (summary()?.missingPriceItemCodes?.length) {
        <div class="alert-banner alert-warning">
          ...
          <div class="alert-content">
            <h4 class="alert-title">Itens com Quantitativo Físico sem Cotação Atribuída (RF-29)</h4>
            <p class="alert-message">
              Os seguintes códigos possuem quantidade calculada maior que zero mas não possuem preço unitário:
              <strong>{{ summary()?.missingPriceItemCodes?.join(', ') }}</strong>.
              O fechamento da revisão da proposta requer resolução dessas pendências.
            </p>
          </div>
        </div>
      }
        ...
        <div class="simulator-panel">
          ...
          <div class="simulator-grid">
            <div class="form-group">
              <label class="form-label" for="simTaxRegime">Regime Tributário (RN-04)</label>
              <select
                id="simTaxRegime"
                class="form-select"
              ...
              >
                ...
                <option value="DIRECT_BILLING">Faturamento Direto ao Cliente</option>
              </select>
            </div>

            <div class="form-group">
              <label class="form-label" for="simLmeUsd">Alumínio LME Spot (USD/t)</label>
              <input
                id="simLmeUsd"
                type="number"
                class="form-input"
            ...
            <div class="form-group">
              <label class="form-label" for="simMidwestUsd">Prêmio Midwest/RTDU (USD/t)</label>
              <input
                id="simMidwestUsd"
                type="number"
                class="form-input"
            ...
            <div class="form-group">
              <label class="form-label" for="simFxRate">Taxa de Câmbio (USD/BRL)</label>
              <input
                id="simFxRate"
                type="number"
                class="form-input"
      ...
      @if (summary()) {
        <div class="kpis-grid">
          ...
          <div class="kpi-card kpi-highlight-tax">
            ...
            <div class="kpi-value text-amber">
              {{ summary()?.totalTaxesAmount | currency:'BRL':'symbol':'1.2-2' }}
            </div>
            <div class="kpi-foot">
              IPI: {{ summary()?.totalIpiAmount | currency:'BRL':'symbol':'1.0-0' }} ·
              DIFAL: {{ summary()?.totalDifalAmount | currency:'BRL':'symbol':'1.0-0' }}
            </div>
          </div>
          ...
          <div class="kpi-card kpi-highlight-total">
            ...
            <div class="kpi-value text-emerald">
              {{ summary()?.totalGrossAmount | currency:'BRL':'symbol':'1.2-2' }}
            </div>
            <div class="kpi-foot">
              Regime: <strong>{{ getTaxRegimeLabel(summary()?.taxRegime) }}</strong>
            </div>
          </div>
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-value text-sky">
              {{ summary()?.totalReidiSavings | currency:'BRL':'symbol':'1.2-2' }}
            </div>
        ...
        <div class="table-card">
          <div class="table-header">
            <h3 class="table-title">Composição Analítica de Materiais e Tributos (RF-34)</h3>
            <span class="table-badge">{{ summary()?.items?.length || 0 }} itens calculados</span>
          </div>
                ...
                @for (item of summary()?.items; track item.itemCode) {
                  <tr>
                    ...
                    <td>
                      ...
                      <div class="text-xs text-slate-500">UF Origem: {{ item.originState }}</div>
                    </td>
                    <td class="text-right font-mono">{{ item.quantity | number:'1.2-2' }}</td>
                    <td class="text-right font-mono">{{ item.netUnitPrice | currency:'BRL':'':'1.2-2' }}</td>
                    <td class="text-right font-mono font-medium">{{ item.netTotalAmount | currency:'BRL':'':'1.2-2' }}</td>
                    ...
                    <td class="text-right font-mono text-slate-600">{{ item.totalIcmsOriginAmount | currency:'BRL':'':'1.2-2' }}</td>
                    <td class="text-right font-mono text-amber-700 font-medium">{{ item.totalDifalAmount | currency:'BRL':'':'1.2-2' }}</td>
                    <td class="text-right font-mono text-slate-600">{{ item.totalFecoepAmount | currency:'BRL':'':'1.2-2' }}</td>
                    <td class="text-right font-mono text-slate-600">
                      {{ (item.pisAmount + item.cofinsAmount) | currency:'BRL':'':'1.2-2' }}
                    </td>
                    <td class="text-right font-mono font-bold text-emerald-800">
                      {{ item.grossTotalAmount | currency:'BRL':'symbol':'1.2-2' }}
                    </td>
        ...
        >
          ...
          >
            <div class="modal-header">
              <div>
                <h3 class="modal-title">Memória de Cálculo Tributário (RF-34)</h3>
            ...
            <div class="modal-body">
              <div class="modal-summary-grid">
                <div class="modal-summary-card">
                  ...
                  <span class="val font-mono">{{ selectedItem()?.quantity | number:'1.2-2' }}</span>
                </div>
                <div class="modal-summary-card">
                  <span class="label">Preço Unitário Líquido:</span>
                  <span class="val font-mono">{{ selectedItem()?.netUnitPrice | currency:'BRL':'symbol':'1.2-2' }}</span>
                </div>
                <div class="modal-summary-card">
                  <span class="label">Base Líquida Total:</span>
                  <span class="val font-mono font-bold">{{ selectedItem()?.netTotalAmount | currency:'BRL':'symbol':'1.2-2' }}</span>
                </div>
                <div class="modal-summary-card">
                  <span class="label">Preço Unitário Final c/ Impostos:</span>
                  <span class="val font-mono font-bold text-emerald-700">{{ selectedItem()?.grossUnitPrice | currency:'BRL':'symbol':'1.2-2' }}</span>
                </div>
              </div>

              <h4 class="section-heading">Detalhamento por Unidade Federativa de Destino (RN-05)</h4>
              <div class="destinations-list">
                @for (dest of selectedItem()?.destinationBreakdowns; track dest.destinationState) {
                  <div class="dest-card">
                    <div class="dest-card-header">
                      <span class="dest-state">Destino: {{ dest.destinationState }}</span>
                      <span class="dest-share">Rateio: {{ dest.sharePercent }}%</span>
                      <span class="dest-method-badge">
                        Método: {{ dest.difalMethod === 'DOUBLE_BASE' ? 'Base Dupla (Reconstituída por dentro)' : 'Base Simples' }}
                      </span>
                    </div>
                    ...
                    <div class="dest-calc-rows">
                      <div class="calc-row">
                        ...
                        <span class="font-mono">{{ dest.allocatedNetBase | currency:'BRL':'symbol':'1.2-2' }}</span>
                      </div>
                      <div class="calc-row">
                        <span>ICMS Origem ({{ dest.interstateRatePercent }}% interestadual):</span>
                        <span class="font-mono">{{ dest.icmsOriginAmount | currency:'BRL':'symbol':'1.2-2' }}</span>
                      </div>
                      @if (dest.difalMethod === 'DOUBLE_BASE') {
                        <div class="calc-row text-sky-800">
                          <span>Base Reconstituída no Destino:</span>
                          <span class="font-mono font-medium">{{ dest.reconstitutedDestinationBase | currency:'BRL':'symbol':'1.2-2' }}</span>
                        </div>
                      }
                      <div class="calc-row font-medium text-amber-800">
                        <span>DIFAL Devido no Destino (Alíquota Interna {{ dest.internalRatePercent }}%):</span>
                        <span class="font-mono">{{ dest.difalAmount | currency:'BRL':'symbol':'1.2-2' }}</span>
                      </div>
                      @if (dest.fecoepRatePercent > 0) {
                        <div class="calc-row text-slate-700">
                          <span>Adicional FECOEP ({{ dest.fecoepRatePercent }}%):</span>
                          <span class="font-mono">{{ dest.fecoepAmount | currency:'BRL':'symbol':'1.2-2' }}</span>
                        </div>
                      }
                    </div>
                  </div>
              ...
              <h4 class="section-heading">Tributos Federais (IPI, PIS & COFINS)</h4>
              <div class="federal-grid">
                <div class="federal-item">
                  <span class="fed-label">IPI ({{ selectedItem()?.ipiRatePercent }}%):</span>
                  <span class="fed-val font-mono">{{ selectedItem()?.ipiAmount | currency:'BRL':'symbol':'1.2-2' }}</span>
                </div>
                <div class="federal-item">
                  <span class="fed-label">PIS ({{ selectedItem()?.pisRatePercent }}%):</span>
                  <span class="fed-val font-mono">{{ selectedItem()?.pisAmount | currency:'BRL':'symbol':'1.2-2' }}</span>
                </div>
                <div class="federal-item">
                  <span class="fed-label">COFINS ({{ selectedItem()?.cofinsRatePercent }}%):</span>
                  <span class="fed-val font-mono">{{ selectedItem()?.cofinsAmount | currency:'BRL':'symbol':'1.2-2' }}</span>
                </div>
                @if (selectedItem()?.reidiBenefitAmount) {
                  <div class="federal-item highlight-reidi">
                    ...
                    <span class="fed-val font-mono text-sky-700 font-bold">
                      -{{ selectedItem()?.reidiBenefitAmount | currency:'BRL':'symbol':'1.2-2' }}
                    </span>
                  </div>
                }
              </div>
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" (click)="closeItemDetail()">Fechar</button>
            </div>
          </div>
        </div>
      }
