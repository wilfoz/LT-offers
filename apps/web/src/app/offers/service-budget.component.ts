...
@Component({
    ...
    <div class="services-container">
      ...
      <div class="header-section">
        <div>
          ...
          <h2 class="title">Orçamento de Serviços e Folhas Contratuais (CIP)</h2>
          <p class="description">
            Consolidação analítica de serviços de transmissão por código CIP, origens de custo, BDI e indicadores paramétricos.
          </p>
        </div>
        ...
        <div class="header-actions">
          @if (lines && lines.length > 0) {
            <div class="line-selector-box">
              <label class="control-label" for="service-line-select">Linha de Transmissão:</label>
              <select
      ...
      @if (loading()) {
        <div class="loading-panel">
          ...
          <span>Consolidando orçamento de serviços e aplicando regras de BDI...</span>
        </div>
      }
      ...
      @if (!loading() && budget(); as b) {
        ...
        <div class="kpis-grid">
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-value highlight">{{ formatCurrency(b.totalSalePrice) }}</div>
            <div class="kpi-subtext">Com BDI aplicado (RF-47)</div>
          </div>

          <div class="kpi-card">
            <div class="kpi-label">Ratio por Extensão</div>
            <div class="kpi-value">{{ formatCurrency(b.ratios.costPerKm) }} <span class="unit">/ km</span></div>
            <div class="kpi-subtext">Venda: {{ formatCurrency(b.ratios.salePricePerKm) }}/km</div>
          </div>

          <div class="kpi-card">
            <div class="kpi-label">Ratio por Estrutura</div>
            <div class="kpi-value highlight-green">{{ formatCurrency(b.ratios.costPerTower) }} <span class="unit">/ torre</span></div>
            <div class="kpi-subtext">Venda: {{ formatCurrency(b.ratios.salePricePerTower) }}/torre</div>
          </div>
        </div>
        ...
        <div class="table-card">
          <div class="table-header">
            <h3 class="table-title">Composição Analítica e Código CIP do Edital (RF-46..RF-48)</h3>
            <span class="badge badge-info">{{ filteredItems().length }} itens listados</span>
          </div>
              ...
              <tbody>
                @for (item of filteredItems(); track item.id) {
                  <tr>
                    ...
                    <td>
                      <span class="cip-badge">{{ item.cipCode || 'S/ CIP' }}</span>
                    </td>
                    <td>
                      <span
                        class="badge"
                        [ngClass]="{
                          'badge-primary': item.costSource === 'SCHEDULE_DIRECT',
                          'badge-warning': item.costSource === 'PARAMETRIC_ADJUSTED',
                          'badge-success': item.costSource === 'SUBCONTRACT_QUOTED'
                        }"
                      >
                    ...
                    <td class="text-right font-mono">{{ formatCurrency(item.unitDirectCost) }}</td>
                    <td class="text-right font-mono font-bold">{{ formatCurrency(item.totalDirectCost) }}</td>
                    <td class="text-center font-mono">{{ item.bdiPercentage }}%</td>
                    <td class="text-right font-mono">{{ formatCurrency(item.unitSalePrice) }}</td>
                    <td class="text-right font-mono font-bold highlight-text">{{ formatCurrency(item.totalSalePrice) }}</td>
                  </tr>
                }
              </tbody>
              <tfoot>
                <tr class="total-row">
                  <td colspan="7" class="text-right font-bold">TOTAL CONSOLIDADO DE SERVIÇOS:</td>
                  <td class="text-right font-bold font-mono">{{ formatCurrency(b.totalDirectCost) }}</td>
                  <td></td>
                  <td></td>
                  <td class="text-right font-bold font-mono highlight-text">{{ formatCurrency(b.totalSalePrice) }}</td>
                </tr>
              </tfoot>
            </table>
          </div>
            ...
            <div class="group-summary-card">
              ...
              <div class="group-costs">
                <div class="cost-row">
                  ...
                  <strong>{{ formatCurrency(b.byGroup[g].totalDirectCost) }}</strong>
                </div>
                <div class="cost-row">
                  <span>Preço Venda:</span>
                  <strong class="highlight-green">{{ formatCurrency(b.byGroup[g].totalSalePrice) }}</strong>
                </div>
  ...
  styles: [`
    ...
    .item-notes { font-size: 0.75rem; color: #64748b; font-weight: 400; margin-top: 0.15rem; }
    .cip-badge {
      display: inline-block;
...
export class ServiceBudgetComponent implements OnInit {
  ...
  getCostSourceLabel(cs: string): string {
    switch (cs) {
      case 'SCHEDULE_DIRECT': return 'Cronograma Direto';
      case 'PARAMETRIC_ADJUSTED': return 'Paramétrico Ajustado';
      case 'SUBCONTRACT_QUOTED': return 'Subcontratado Cotado';
      default: return cs;
    }
  }
