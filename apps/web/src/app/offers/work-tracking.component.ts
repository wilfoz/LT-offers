    ...
    <div class="work-tracking-container">
      ...
      <div class="tracking-header">
        <div class="header-info">
          ...
          <div>
            ...
            <p class="subtitle">
              Linha de Base Contratual Imutável (Data 0), Curva S (Previsto vs.
              Realizado), Análise de Valor Agregado (IDP/IDC), Aditivos e Carga
              ERP
            </p>
          </div>
        </div>
      ...
      @if (loading()) {
          ...
          <div class="baseline-banner">
            ...
            <div class="banner-grid">
              <div class="stat-box">
                ...
                <span class="value">{{
                  formatCurrency(baseline()?.totalContractValue)
                }}</span>
              </div>
              <div class="stat-box">
                <span class="label">Custo Orçado da Linha</span>
                <span class="value">{{
                  formatCurrency(baseline()?.totalBudgetCost)
                }}</span>
              </div>
              <div class="stat-box">
                <span class="label">Margem Alvo Planejada</span>
                <span class="value highlight"
                  >{{ baseline()?.targetMarginPercent }}%</span
                >
              </div>
              <div class="stat-box">
                <span class="label">Prazo de Execução</span>
                <span class="value"
                  >{{ baseline()?.scheduleMonths }} Meses</span
                >
              </div>
              <div class="stat-box">
                <span class="label">Congelado Em</span>
                <span class="value small">{{
                  formatDate(baseline()?.frozenAt)
                }}</span>
              </div>
            </div>
          </div>
        ...
        <div class="kpi-cards-grid">
          ...
          <div
            class="kpi-card"
            [class.danger]="isCpiLow()"
            [class.success]="!isCpiLow()"
          >
            <div class="kpi-icon">
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-content">
              ...
              <span class="kpi-value highlight"
                >{{
                  curveSData()?.currentPhysicalProgressPercent || '0.00'
                }}%</span
              >
          ...
          <div class="kpi-card highlight-card">
            ...
            <div class="kpi-content">
              ...
              <span class="kpi-value">{{
                formatCurrency(cweData()?.currentWorkingEstimateValue)
              }}</span>
              <span class="kpi-subtext"
                >+{{ formatCurrency(cweData()?.totalApprovedAdditivesCost) }} em
                Aditivos</span
              >
            </div>
          </div>
        ...
        <mat-card class="curve-s-card">
          <mat-card-header>
            <mat-card-title>
              ...
              Curva S de Acompanhamento Físico-Financeiro (Previsto vs. Agregado
              vs. Realizado)
            </mat-card-title>
            <mat-card-subtitle>
              Status Consolidado:
              <span
                class="status-pill"
                [class]="curveSData()?.statusSummary?.toLowerCase()"
              >
            ...
            <div class="chart-wrapper">
              ...
              <div class="chart-legend">
                <div class="legend-item">
                  <span class="legend-box pv"></span> Valor Planejado ($PV$ -
                  Baseline)
                </div>
                <div class="legend-item">
                  <span class="legend-box ev"></span> Valor Agregado ($EV$ -
                  Físico Realizado)
                </div>
                <div class="legend-item">
                  <span class="legend-box ac"></span> Custo Real ($AC$ - Medição
                  Financeira)
                </div>
              </div>
            ...
            <div class="tab-content">
              <div class="table-header-bar">
                ...
                <span class="count-tag"
                  >{{ baseline()?.workPackages?.length || 0 }} Pacotes
                  Contratados</span
                >
              </div>
                ...
                <tbody>
                  @for (wp of baseline()?.workPackages; track wp.wbsCode) {
                    <tr>
                      ...
                      <td class="text-right">
                        <strong class="highlight-pct"
                          >{{ wp.weightPercent }}%</strong
                        >
                      </td>
                    </tr>
                  }
                ...
                <tbody>
                  @for (rec of progressRecords(); track rec.monthNumber) {
                    <tr>
                      ...
                      <td class="text-right">
                        <strong>{{ rec.physicalProgressPercent }}%</strong>
                      </td>
                      <td class="text-right currency">
                        {{ formatCurrency(rec.monthlyMeasuredAmount) }}
                      </td>
                      <td class="text-right currency">
                        {{ formatCurrency(rec.earnedValueCumulative) }}
                      </td>
                      <td class="text-right currency">
                        {{ formatCurrency(rec.actualCostCumulative) }}
                      </td>
                      <td>{{ rec.createdBy }}</td>
          ...
          <mat-tab label="Aditivos & Pleitos (Change Orders)">
            <div class="tab-content">
              <div class="table-header-bar">
                <div>
                  <h3>
                    Gestão de Ordens de Alteração Contratual (*Change Orders*)
                  </h3>
                  <p class="sub">
                    Projeção CWE:
                    {{ formatCurrency(cweData()?.currentWorkingEstimateValue) }}
                ...
                <tbody>
                  @for (co of changeOrders(); track co.code) {
                    <tr>
                      ...
                      <td>
                        <span class="category-chip">{{
                          getChangeOrderTypeLabel(co.type)
                        }}</span>
                      </td>
                      <td>
                        <span
                          class="status-badge"
                          [class]="co.status.toLowerCase()"
                        >
                          {{ getChangeOrderStatusLabel(co.status) }}
                        </span>
                      </td>
                      <td class="text-right currency">
                        {{ formatCurrency(co.requestedCostDelta) }}
                      </td>
                      <td class="text-right currency highlight">
                        {{ formatCurrency(co.approvedCostDelta) }}
                      </td>
                      <td class="text-center">
                        +{{ co.scheduleDeltaMonths }}m
                      </td>
                      <td class="text-center">
            ...
            <div class="tab-content">
              <div class="table-header-bar">
                <div>
                  ...
                  <p class="sub">
                    Geração de pacotes padronizados para SAP S/4HANA, TOTVS/RM e
                    Sienge/Mega
                  </p>
                </div>
                ...
                <div class="erp-actions">
                  <mat-form-field appearance="outline" class="system-select">
                    ...
                    <mat-select [(ngModel)]="selectedErpSystem">
                      ...
                      <mat-option value="GENERIC_JSON"
                        >Universal JSON</mat-option
                      >
                    </mat-select>
                  </mat-form-field>
              ...
              <div class="erp-summary-card">
                ...
                <p>
                  O pacote contempla o mapeamento de Centros de Custo por
                  trecho, contas contábeis do Razão e distribuição mensal
                  físico-financeira.
                </p>
                <div class="erp-pills">
                  <span class="pill"
                    ><mat-icon>domain</mat-icon> Código Empresa: 1000 /
                    COLIG-01</span
                  >
                  <span class="pill"
                    ><mat-icon>layers</mat-icon> 7 Centros de Custo EAP</span
                  >
                  <span class="pill"
                    ><mat-icon>calendar_today</mat-icon> 18 Meses de
                    Distribuição</span
                  >
                </div>
              </div>
      ...
      @if (showFreezeModal()) {
        <div class="modal-overlay">
          <div class="modal-card">
            ...
            <p class="modal-desc">
              Esta ação tornará a revisão da proposta vencedora imutável e
              registrará a baseline contratual definitiva para medições e gestão
              de aditivos.
            </p>
            ...
            <div class="modal-actions">
              ...
              <button mat-flat-button color="accent" (click)="confirmFreeze()">
          ...
          <div class="modal-card">
            ...
            <div class="form-grid">
              ...
              <mat-form-field appearance="outline">
                ...
                <input
                  matInput
                  [(ngModel)]="newProgressMeasuredAmount"
                  placeholder="ex: 8500000.00"
                />
              </mat-form-field>
            </div>
            ...
            <mat-form-field appearance="outline" class="full-width">
              ...
              <textarea
                matInput
                [(ngModel)]="newProgressNotes"
                rows="2"
              ></textarea>
            </mat-form-field>

            <div class="modal-actions">
              <button mat-button (click)="closeProgressModal()">
                Cancelar
              </button>
              <button
                mat-flat-button
                color="primary"
                (click)="confirmProgressRecord()"
              >
                Salvar Boletim
              </button>
            </div>
        ...
        <div class="modal-overlay">
          <div class="modal-card">
            ...
            <div class="form-grid">
              ...
              <mat-form-field appearance="outline">
                ...
                <mat-select [(ngModel)]="newCoType">
                  <mat-option value="GEOTECHNICAL_SOIL"
                    >Geotecnia / Solo Imprevisto</mat-option
                  >
                  <mat-option value="ALIGNMENT_TOWER_RELOCATION"
                    >Realocação de Estruturas</mat-option
                  >
                  <mat-option value="ENVIRONMENTAL_REQUISITION"
                    >Exigência Ambiental</mat-option
                  >
                  <mat-option value="PRICE_READJUSTMENT"
                    >Reajuste de Preços</mat-option
                  >
                  <mat-option value="SCOPE_ADDITION"
                    >Aditivo de Escopo</mat-option
                  >
            ...
            <mat-form-field appearance="outline" class="full-width">
              ...
              <textarea
                matInput
                [(ngModel)]="newCoJustification"
                rows="2"
              ></textarea>
            </mat-form-field>

            <div class="modal-actions">
              <button mat-button (click)="closeChangeOrderModal()">
                Cancelar
              </button>
              <button
                mat-flat-button
                color="accent"
                (click)="confirmCreateChangeOrder()"
              >
                Cadastrar Aditivo
              </button>
            </div>
...
export class WorkTrackingComponent implements OnInit {
  ...
  // Form fields
  freezeNotes =
    'Linha de Base Contratual Data 0 congelada a partir da proposta vencedora.';
  newProgressMonth = 4;
  ...
  newCoJustification =
    'Sondagens complementares demonstraram camada de argila mole de 12m.';
  ...
  readonly pvPoints = computed(() => {
    ...
    return series
      .map((s, idx) => {
        ...
        const progressPct =
          parseFloat(s.plannedValue) /
          (parseFloat(this.baseline()?.totalContractValue || '1') || 1);
  ...
  confirmProgressRecord(): void {
    ...
    this.baselineApi
      .recordMonthlyProgress(this.offerId() || 1, payload)
      .subscribe({
        next: () => {
  ...
  getStatusLabel(status?: string): string {
    return (
      (status && (CURVE_S_STATUS_LABELS as any)[status]) || status || 'No Prazo'
    );
  }
}
