...
import { GroundWiresUseCases } from '../../../application';
import {
  createIdPipe,
...
export class GroundWiresController {
  constructor(private readonly useCases: GroundWiresUseCases) {}

  @Post()
  async create(
    @Body() dto: CreateGroundWireDto,
    ...
    try {
      ...
      return CatalogPresenter.toGroundWireSummary(
        entity,
        resolveReferenceDate(),
      );
    } catch (error) {
  ...
  async list(
    ...
    try {
      ...
      return items.map((item) =>
        CatalogPresenter.toGroundWireSummary(item, refDate),
      );
    } catch (error) {
      handleCatalogDomainError(error);
    }
  ...
  async get(
    ...
    try {
      ...
      return CatalogPresenter.toGroundWireSummary(entity, refDate);
    } catch (error) {
      handleCatalogDomainError(error);
    }
  }
  ...
  async listHistory(@Param('id', IdPipe) id: number) {
    try {
      ...
      return CatalogPresenter.toGroundWireHistory(entity);
    } catch (error) {
      handleCatalogDomainError(error);
    }
  }
  ...
  async createVersion(
    ...
    try {
      ...
      return CatalogPresenter.toGroundWireVersion(version);
    } catch (error) {
      handleCatalogDomainError(error);
    }
  }
