...
export class PricingService {
  ...
  async calculateLinePricing(
    ...
    const icmsRulesMap = this.taxTablesService.getIcmsRulesMap();
    const ipiRulesMap = this.taxTablesService.getIpiRulesMap();
    const pisCofinsRule = this.taxTablesService.getPisCofinsRule(taxRegime);
  ...
  private async buildMaterialPricingItems(
    ...
    try {
      const foundationSummary =
        await this.foundationsService.getLineFoundationSummary(lineId);
