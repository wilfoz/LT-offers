...
export class ServiceBudgetService {
  ...
  async getLineServiceBudget(lineId: number): Promise<ServiceBudgetSummary> {
  ...
  async getLineMeasurementSheet(lineId: number): Promise<ServiceBudgetItem[]> {
    const summary = await this.getLineServiceBudget(lineId);
